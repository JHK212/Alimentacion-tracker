...
- Migración rev 12: setea `until` en la rutina vieja y hace unshift de `rutina-if`. Rev 13: reemplaza `meals` de `rutina-if` in-place con la versión final (seguro porque aún no tenía días logueados — si hiciera falta otro reemplazo DESPUÉS del estreno, va con `until` + scenario nuevo, no in-place). Rev 14 (`PLANS_REV=14`): mismo reemplazo in-place el día del estreno (barrita → panqueque). El bloque rev-6 ahora unshiftea TODOS los `DEFAULT_SCENARIOS` (la nueva + la vieja con until) para que un device fresco también resuelva historia.
