...
- Un solo scenario activo: `rutina` (4 comidas, ~1770 kcal, ~204g prot). Los 4 scenarios viejos quedan en `food-plans` con `legacy:true` — el historial pre-cutover los sigue leyendo. No borrarlos.
...
- `merienda` 17:30 "(laburo)" — mugcake + barrita proteica. Restricción real: tiene que viajar en mochila (el yogur no llega al trabajo). Alt "comprado" (yogur bebible + barrita + banana) para el día que se olvida el mugcake — ya le pasó.
...
- Migración rev 12: setea `until` en la rutina vieja y hace unshift de `rutina-if`. Rev 13 (`PLANS_REV=13`): reemplaza `meals` de `rutina-if` in-place con la versión final (seguro porque aún no tenía días logueados — si hiciera falta otro reemplazo DESPUÉS del estreno, va con `until` + scenario nuevo, no in-place). El bloque rev-6 ahora unshiftea TODOS los `DEFAULT_SCENARIOS` (la nueva + la vieja con until) para que un device fresco también resuelva historia.
